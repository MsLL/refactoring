...
class Customer():
    ...
    def statement(self):
        ...
        for rental in self.rental:
            result += "{title}\t{cost}\n".format(
                ...
                cost=str(MovieServiceFactory.getMovieService(rental.movie.type).getCost(rental.daysRented))
            )
        result += "Amount owed is {totalCost}\n".format(totalCost=str(self.getTotalCost()))
        result += "You earned {frequentRenterPoints} frequent renter points".format(
            frequentRenterPoints=str(self.getTotalFrequentRenterPoints()))
        print(result)
    ...
    def getTotalFrequentRenterPoints(self):
        ...
        for rental in self.rental:
            frequentRenterPoints += MovieServiceFactory.getMovieService(rental.movie.type).getFrequentRenterPoints(
            rental.daysRented)
